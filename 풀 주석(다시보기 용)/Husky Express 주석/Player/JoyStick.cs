...
public class JoyStick : MonoBehaviour, IDragHandler,IPointerUpHandler,IPointerDownHandler {
    ...
    void Start () {
        bgImg = GetComponent<Image>();                              //백그라운드 이미지를 받아옵니다
        joystickImg = transform.GetChild(0).GetComponent<Image>();  //조이스틱 이미지를 받아옵니다
	}
    ...
    public virtual void OnPointerDown(PointerEventData ped)//화면에 터치를 하는 순간
    {
        ...
        OnDrag(ped);                    //메인캠에서 활성화를 시키고 드래그를 이벤트를 시작합니다
    }
    ...
    public float GetHorizontalValue()//조이스틱의 Horizontal값 x값
    {
        return inputVector.x;
    }
    public float GetVerticalValue()//조이스틱의 Vertical값 y값
    {
