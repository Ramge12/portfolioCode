...
public class GameClient2 : MonoBehaviour {
    ...
    public enum eState { Connecting, LoggingOn, InVille, Failed }   //네트워크 상태문
    HostID m_p2pGroupID = HostID.HostID_None;                       //Host ID를 지정하는 부분
    public MulityPlay m_MulityPlay;                                 //멀티플레이 클래스를 받아오는 부분
    public GameObject player_selected;                              //플레이어 1,2를 구분하는 오브젝트
    public eState m_eState;                                         //네트워크 상태문입니다
    ...
    void Update()
    {
        if (GameObject.Find("PlayerNum"))                   //PlayerNum을 찾아서
        {
            player_selected = GameObject.Find("PlayerNum"); //플레이어 번호를 select에 적용합니다
        }
        switch (m_eState)
        {
            case eState.InVille:
