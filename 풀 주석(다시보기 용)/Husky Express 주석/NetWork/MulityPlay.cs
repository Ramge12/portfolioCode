...
public class MulityPlay : MonoBehaviour {
    ...
    public Vector3 Player1_position;        //플레이어 1의 포지션값
    public Vector3 Player2_position;        //플레이어 2의 포지션값
    public float Player1_rotation;          //플레이어 1의 회전값
    public float Player2_rotation;          //플레이어 2의 회전값
    public int Player1_run = 0;             //플레이어 1의 달렸는지에 대한 여부
    public int Player2_run = 0;             //플레이어 2의 달렸는지에 대한 여부
    ...
    void Start ()
    {
        ...
        {
            player_selected = GameObject.Find("PlayerInfo");    //PlayerInfo를 찾아 player_selected에 넣습니다
        }
        if (player_selected.GetComponent<PlayerSelectInfo>().player_select_Num == 1)    //플레이어1의 경우
        {
        ...
        if (player_selected.GetComponent<PlayerSelectInfo>().player_select_Num == 2)//플레이어2일 경우
        {
    ...
    void Update()
    {
        if (Player_1_check == true && Player_2_check == true && goal == false) //goal라인에 아무도 들어오지않았고 Player1,2가 모두 접속했을때
        {
            mulityTimer += Time.deltaTime;                              //출발시간을 재기위해 시간을 잽니다
            m_info_text.text = "출발 까지 남은시간:" + ((int)(3.0f - mulityTimer)).ToString();
            if (mulityTimer > 3.0f)                                        //출발 시간은 3초이고 3초가 경과한다면
            {
                player1_rec.RecordMode = true;      //Player1 레코드 활성화
                player2_rec.RecordMode = true;      //Player2 레코드 활성화
                m_info_UI.SetActive(false);         //출발시간을 재는 UI비활성화
                P2_PengJoyStic.MoveSpeed = 5.0f;    //플레이어1과 2의 이동속도를 줍니다
                P1_DogJoyStic.MoveSpeed = 5.0f;
            }
        }
        if (player_selected.GetComponent<PlayerSelectInfo>().player_select_Num == 1)//플레이어 넘버가 1인경우(플레이어1인경우)
        {
        ...
        if (player_selected.GetComponent<PlayerSelectInfo>().player_select_Num == 2)//플레이어 넘버가 2인경우(플레이어2인경우)
        {
    ...
    void Set_Joystick()//보내기(원래는 조이스틱정보를 보냈으나, 위치정보로 수정함;;)
    {
        ...
        player_select.text = player_selected.GetComponent<PlayerSelectInfo>().player_select_Num.ToString(); //플레이어 1인지 2인지 text UI에 출력합니다
        p1_position.text = player_1Slide.transform.position.ToString();                     //플레이어1의 위치 출력 UI
        p2_position.text = player_2Slide.transform.position.ToString();                     //플레이어2의 위치 출력 UI
    }
    public void Record_player()//멀티 플레이에서 레코드 라인을 구분하는 하여 기록하는 함수
    {
    ...
    public void Record_time_score()//멀티 플레이에서의 기록을 저장하는 함수
    {
        if (player_selected.GetComponent<PlayerSelectInfo>().player_select_Num == 1)
        {
