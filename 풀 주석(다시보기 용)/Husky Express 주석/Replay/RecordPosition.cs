...
public class RecordPosition : MonoBehaviour {
    ...
    public PlayerChractor m_player_info;                    //플레이어 정보
    public FileInput File_input;                            //파일 입출력매니저
    public float replayTime = 0;                            //리플레이 총시간
    public bool RecordMode = false;                         //레코드 모드에 대한 불 값
    ...
    {
        RecordMode = false;     //기록정지
    }
    public void Write_Record()//지금까지한 기록을 모두 파일에 저정합니다.
    {
        //파일 매니저를 통해 txt에 데이터를 모두 저장합니다.
